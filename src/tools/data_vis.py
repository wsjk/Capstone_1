...
def get_plotly(dfs, x, y="", dfname=[], xlabel='', ylabel="", text = 'title', size="", plot_type='scatter', title=""):
    if plot_type == 'scatter':
        ...
        layout = go.Layout(title=title if title else x + ' vs ' + y,
        ...
        data = [go.Histogram(x=df[x],
                        name=dfname) for df, dfname in zip(dfs,dfname)]
